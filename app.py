...
def infer_periods(entries):
    ...
    for i in range(len(entries)):
        ...
        if i + 1 < len(entries):
            ...
            end_time = ""
        data.append([start_time, end_time, activity, "", logged_at])
    df = pd.DataFrame(data, columns=["Start", "End", "Activity", "Comments", "Logged at"])
    return df
...
if os.path.exists("activities.csv"):
    ...
    df_existing = pd.DataFrame(columns=["Start", "End", "Activity", "Comments", "Logged at"])
    ...
    if st.button("🎙️ Voice Input"):
        ...
        if text:
            ...
            if time_str:
                ...
                st.session_state.entries.append((time_str, activity, logged_at))
            else:
...
if st.session_state.entries:
    ...
    st.dataframe(pd.DataFrame(st.session_state.entries, columns=["Time", "Activity", "Logged at"]).sort_values(by="Start"))
